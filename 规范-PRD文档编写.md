...
*   **字段定义表**：
    - 字段定义必须同时锁定“前端字段名（camelCase）/数据库字段名（snake_case）/数据库类型”，用于 AI 确定性生成 DTO、VO、表结构与注释。
    | 字段名称 | 字段类型 | 必填 | 默认值 | 校验规则 | 前端字段名 | 数据库字段名 | 数据库类型（MySQL） | 备注 |
    | :--- | :--- | :---: | :--- | :--- | :--- | :--- | :--- | :--- |
    | SKU编码 | 文本 | ✅ | 无 | 唯一性校验，不支持特殊字符 | skuCode | sku_code | VARCHAR(64) | 自动转大写 |
    | 价格 | 金额 | ✅ | 0.0000 | >0，保留2位小数 | price | price | DECIMAL(10,4) | 单位：元 |

**字段类型标准化定义**（AI必须严格遵循）：
| 类型 | 后端 Java | 后端 Python | 前端 Vue/React | 数据库（MySQL 5.7+） | 固定约束（禁止模糊） |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 文本 | `String` | `str` | `string` | `VARCHAR(128)` | 默认长度 128；如超长必须改用“备注/长文本”或明确长度 |
| 编码/业务单号 | `String` | `str` | `string` | `VARCHAR(64)` | 如需唯一，必须在 PRD 中增加唯一约束与唯一索引 |
| 手机号 | `String` | `str` | `string` | `VARCHAR(32)` | 允许国际区号；禁止使用数值类型 |
| 邮箱 | `String` | `str` | `string` | `VARCHAR(128)` | 必须提供格式校验规则 |
| 金额（含小数） | `BigDecimal` | `Decimal` | `number` | `DECIMAL(10,4)` | 默认 4 位小数；如业务要求 2 位小数，校验规则必须写明但落库仍为 4 位 |
| 整数 | `Integer` | `int` | `number` | `INT` | 默认值必须明确（如 0/1/10） |
| 布尔 | `Boolean` | `bool` | `boolean` | `TINYINT(1)` | 固定用 0/1 存储；注释必须写“0否；1是” |
| 日期 | `LocalDate` | `date` | `Date` | `DATE` | 只表达日期，不包含时分秒 |
| 时间戳 | `LocalDateTime` | `datetime` | `Date` | `DATETIME` | 业务时间字段名必须以 `_time` 结尾 |
| 列表（字符串列表） | `List<String>` | `list[str]` | `string[]` | `JSON` | 仅用于低频查询字段；如需按元素查询必须冗余字段并建索引 |
| JSON对象 | `String` 或 `Map<String,Object>` | `dict` | `Record<string, any>` | `JSON` | 禁止对 JSON 路径做高频查询；如需查询必须冗余为普通列 |
| 备注 | `String` | `str` | `string` | `VARCHAR(512)` | 可为空 |
| 长文本/富文本 | `String` | `str` | `string` | `TEXT` | 富文本内容必须标注是否包含 HTML |

**数据库通用字段（强制）**：
- 所有业务表必须包含（并在 PRD 的数据字典中出现）：`status`、`is_deleted`、`create_time`、`last_modified_time`，字段定义与默认值必须遵循 [规范-03-数据库设计规范.md](规范-03-数据库设计规范.md)。
...
- **页面与字段**：
  | 字段名称 | 字段类型 | 必填 | 默认值 | 校验规则 | 前端字段名 | 数据库字段名 | 数据库类型（MySQL） | 备注 |
  | :--- | :--- | :---: | :--- | :--- | :--- | :--- | :--- | :--- |
  | 邮箱 | 邮箱 | ✅ | 无 | 正则 ^[^\s@]+@[^\s@]+\.[^\s@]+$ | email | email | VARCHAR(128) | - |
  | 密码 | 文本 | ✅ | 无 | 长度 8-20 位 | password | password_hash | VARCHAR(128) | 只存 hash，不落库明文 |
- **交互逻辑**：
