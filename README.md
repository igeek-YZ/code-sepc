...
本项目是一个面向 AI 时代的 **高精度研发规范库**。我们将模糊的业务需求转化为机器可理解、人可审计的标准化文档，从而让 AI（如 Trae, Claude, GPT）能够生成几乎零偏差的代码。
...
| **全局定义** | [全局定义](./规范-00-全局定义.md) | 技术栈、系统环境、全局变量、非功能需求。 |
| **后端开发** | [后端开发规范](./规范-01-后端开发规范.md) | 架构分层、代码规范、日志/异常/配置/测试等标准。 |
| **接口定义** | [API 接口定义规范](./规范-API接口定义.md) | RESTful API 定义、字段表格化及交互契约。 |
| **网页前端开发** | [网页前端开发规范（控制台）](./规范-02-前端开发规范.md) | Vue 3 + Element Plus 控制台工程结构、路由/鉴权/接口/页面模板。 |
...
-   **[system-architect](./.trae/skills/system-architect/SKILL.md)**:
    -   **职责**：根据设计稿锁定 [API定义](./规范-API接口定义.md) 和技术规范（[全局定义](./规范-00-全局定义.md)、[后端开发规范](./规范-01-后端开发规范.md) 等）。
...
在运行 `scripts/` 目录下的自动化部署脚本前，请确保已在 [规范-00-全局定义](./规范-00-全局定义.md) 的 **全局定义** 章节中配置了以下关键变量：
