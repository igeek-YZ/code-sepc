...
在开始部署前，用户需要提供基础部署信息。这些配置已统一在 [规范-技术栈&系统环境.md](./规范-技术栈&系统环境.md) 的 **全局定义** 中维护。

AI 将读取这些全局变量，用于自动修改项目的 `application-prod.yml`、`Nginx` 配置以及 `scripts` 脚本。
...
2.  **脚本生成**:
    *   AI 根据部署路径和服务器信息，在 `scripts/` 目录下生成或更新 `deploy-backend.sh` 和 `deploy-frontend.sh`。
3.  **构建产物**:
