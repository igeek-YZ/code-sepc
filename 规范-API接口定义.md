...
- 本规范用于让 AI 依据“接口契约 + [规范-01-后端开发规范.md](规范-01-后端开发规范.md)”生成后端与前端代码时做到：**无偏移、无幻觉、无猜测**。

### 0.1 禁止猜测（硬性）

- 接口文档必须表格化锁定所有字段：字段名、类型、是否必填、默认值、校验规则、示例值、错误码与错误提示。
- 接口文档未声明的任何内容（字段/枚举/错误码/分页协议/排序/鉴权）：
  - AI 必须输出“缺失信息清单”并停止生成，禁止自行补全或使用 `xxx`/`TODO`/`可能`。

### 0.2 与后端规范对齐（硬性）

- **统一响应结构**：必须使用 `Result<T>`，字段固定为 `code`、`msg`、`data`。
- **成功码**：固定为 `"200"`（字符串）。
- **失败码**：必须为 6 位数字字符串业务错误码（禁止使用 HTTP 状态码当业务错误码）。
- **HTTP 状态码**：默认统一返回 200；业务失败通过 `code != "200"` 表达。
- **字段命名**：JSON 字段必须 camelCase；数据库字段为 snake_case（通过 Entity `@TableField` 映射）。
- **分页默认协议**：除非接口明确声明使用 MyBatis-Plus Page 的 pageNo/pageSize，否则统一使用“游标分页”（lastId/pageSize，按 id DESC）。
...
6.  **响应结果 (Response)**：
    *   成功响应的 JSON 结构示例。
    *   包含标准的状态码 (`code`)、提示信息 (`msg`) 及数据载荷 (`data`)。
    *   **字段说明**：
        *   `code`：状态码，`"200"` 表示成功，其他 6 位数字字符串表示业务错误。
        *   `msg`：对状态码的详细描述，用于客户端展示。
        *   `data`：业务数据载荷；错误响应中必须为 null。

7.  **数据与副作用清单（必须）**：用于让 AI 生成代码不偏移：
    *   **数据清单**：涉及的表名（含 `{{TABLE_PREFIX}}`）与关键字段（如需落库）。
    *   **Redis 清单**：Key、过期时间、读写位置（如使用）。
    *   **事件清单**：事件类名、发布点、监听器、是否异步（如使用）。
---
...
### 2. URL 设计
*   **风格**：RESTful 风格。
*   **端区分前缀（固定，禁止改名）**（来源：[规范-01-后端开发规范.md](规范-01-后端开发规范.md)）：
    | 端 | URL 前缀 | 示例 |
    |---|---|---|
    | B端管理后台 | `/admin/{module}` | `/admin/order` |
    | C端业务接口 | `/api/{module}` | `/api/order` |
    | OpenAPI | `/open/{module}` | `/open/order` |
*   **版本号**：
    - 默认 **不在 URL 里引入版本号**（避免 `/api/v1` 与端前缀规则冲突）。
    - 如必须引入版本号，需在接口文档“接口地址”处明确写出，并在同一项目内全量一致（否则视为缺失信息，AI 必须停止生成）。
*   **资源命名与操作分层**：
    | 类型 | HTTP 方法 | URL 模式 | 示例 | 说明 |
    |------|-----------|----------|------|------|
    | 资源集合查询 | GET | `/{prefix}/{plural-resource}` | `GET /api/products` | 查询资源列表 |
    | 单个资源查询 | GET | `/{prefix}/{plural-resource}/{id}` | `GET /api/products/123` | 查询指定资源 |
    | 关联资源查询 | GET | `/{prefix}/{plural-resource}/{id}/{child-resource}` | `GET /api/products/123/comments` | 查询资源的子资源 |
    | 特定操作 | POST | `/{prefix}/{resource}/{action}` | `POST /api/orders/cancel` | 业务特定操作（必须用 POST） |
    | 认证登录 | POST | `/{prefix}/auth/{action}` | `POST /api/auth/login` | 特殊前缀 auth |
*   **路径参数**：使用 `{paramName}` 表示动态路径参数（如 `/api/products/{skuCode}`）。
*   **Query 参数**：用于筛选、排序、分页。
    *   分页（默认游标分页）：`lastId`, `pageSize`（默认 `orderBy=id DESC`）
...
*   **示例**：`GET /api/products?lastId=0&pageSize=20&status=ON_SALE`
*   **字段说明**：
    | 参数名 | 类型 | 必填 | 默认值 | 示例值 | 描述 |
    | :--- | :--- | :--- | :--- | :--- | :--- |
    | `lastId` | Long | 否 | 0 | 1000 | 游标（按 id DESC；首次请求固定 0） |
    | `pageSize` | Integer | 否 | 20 | 20 | 每页条数（最大值必须明确，默认 100） |
...
*   **字段说明**：
    ...
    | `price` | BigDecimal | 是 | >0, 最多4位小数 | 价格（单位：元） |
...
*   **异常场景**：
    | 场景 | 错误码 | 错误提示 | 处理建议 |
    | :--- | :--- | :--- | :--- |
    | 库存不足 | 202000 | 商品库存不足 | 提示用户减少数量 |
    | 支付失败 | 509000 | 支付失败 | 提示用户稍后重试或更换支付方式 |
...
*   **校验层级**：
    | 层级 | 校验方式 | 适用场景 | 错误码 |
    |------|----------|----------|--------|
    | Controller 层 | `@Valid` + JSR-303 注解 | 格式校验（非空、长度、正则） | 000100 |
...
*   **验证规则结构化格式**（API 接口文档必须使用）：
    ...
    | `price` | 数值 | > 0, 最多4位小数 | 200102 | 价格必须大于0 |
    | `skuCode` | 唯一性 | 查询 product 表 | 203000 | 商品编码已存在 |
...
### 6.1 错误码规范
*   **错误码格式**：`{模块码}{类型码}{序号}`，固定 6 位数字字符串（模块码2位 + 类型码2位 + 序号2位）。
*   **模块码定义**：
    | 模块 | 模块码 | 示例 |
    |------|--------|------|
...
*   **异常场景表格模板**：
    ...
    | :--- | :--- | :--- | :--- |
    | 库存不足 | 202000 | 商品库存不足 | 提示用户减少数量 |
    | 用户不存在 | 101000 | 用户不存在 | 检查用户ID是否正确 |
    | Token无效 | 605000 | Token无效，请重新登录 | 跳转登录页 |
...
*   **字段规范**：
    | 字段名 | 类型 | 说明 | 必填/可选 |
    |--------|------|------|----------|
    | `code` | String | 状态码，"200" 表示成功，其他 6 位数字字符串为业务错误 | 必填 |
...
1.  **对象边界 (Object Boundary)**：
    ...
    *   必须提供 Entity 与 DTO/VO 之间的转换代码（统一使用手写 Convert 类，命名：`XxxConvert`，禁止依赖未声明的第三方库）。

2.  **分页结构适配**：
    *   默认必须使用“游标分页”（Query：`lastId`、`pageSize`；排序：`ORDER BY id DESC`）。
    *   所有分页查询接口必须在文档中明确声明：
        - 是否使用游标分页（默认是）
        - 排序字段（默认 `id DESC`，如需变更必须写明）
    *   建议定义通用 `PageResult<T>` 包装类：
        ```java
        @Data
        ...
        ```
    *   如接口明确声明使用 pageNo/pageSize（传统分页），必须在 Query 参数表中给出 `pageNo`、`pageSize` 并说明排序字段；否则视为缺失信息，AI 必须停止生成。
...
#### 1. {接口名称} API
- **接口**：`{METHOD} {URL}`（如：`POST /api/order/create`，或 `GET /admin/order/page`）
...
- **请求体字段**：
    ...
    | :--- | :--- | :--- | :--- | :--- | :--- |
    | `field1` | String | 是 | max:50 | 000100 | 字段1不能为空 |
- **验证规则（结构化）**：
    | 字段名 | 规则类型 | 具体约束 | 错误码 | 错误提示 |
    |--------|----------|----------|--------|----------|
    | `field1` | 非空 | - | 000100 | 字段1不能为空 |
    | `field2` | 格式 | 正则：^\d+$ | 000100 | 字段2格式不正确 |
- **异常场景**：
| 场景 | 错误码 | 错误提示 | 处理建议 |
| :--- | :--- | :--- | :--- |
| 找不到数据 | 101000 | 数据不存在 | 检查ID是否正确 |
- **响应（操作类）**：
    ```json
    {
      "code": "200",
...
- **请求体字段**：
    | 字段名 | 类型 | 必填 | 约束 | 错误码 | 错误提示 |
    | :--- | :--- | :--- | :--- | :--- | :--- |
    | `skuCode` | String | 是 | max:50 | 000100 | 商品编码不能为空 |
    | `skuName` | String | 是 | max:100 | 000100 | 商品名称不能为空 |
    | `brand` | String | 是 | max:50 | 000100 | 品牌不能为空 |
    | `price` | BigDecimal | 是 | >0, 最多4位小数 | 000100 | 价格必须大于0 |
    | `totalStock` | Integer | 是 | >=0 | 000100 | 库存必须大于等于0 |
- **验证规则（结构化）**：
    | 字段名 | 规则类型 | 具体约束 | 错误码 | 错误提示 |
    |--------|----------|----------|--------|----------|
    | `skuCode` | 唯一性 | 查询 product 表 | 203000 | 商品编码已存在 |
    | `price` | 数值 | > 0 | 000100 | 价格必须大于0 |
    | `totalStock` | 数值 | >= 0 | 000100 | 库存必须大于等于0 |
- **异常场景**：
    | 场景 | 错误码 | 错误提示 | 处理建议 |
    | :--- | :--- | :--- | :--- |
    | 商品编码已存在 | 203000 | 商品编码已存在，请使用其他编码 | 提示用户更换编码 |
    | 参数校验失败 | 000100 | 参数校验失败 | 检查必填字段 |
- **响应（操作类）**：
    ```json
    {
      "code": "200",
